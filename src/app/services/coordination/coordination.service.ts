...
export class CoordinationService {
  ...
  async welcomeUser() {
    ...
    await this.step('welcome', 'sendMessage', {
      text: 'My name is Mila. I am thrilled to be working with you today.', position:
        'center'
    });

    await this.step('welcome', 'sendMessage', {
      text: "I am here to guide you through today's session.", position:
        'center'
    }, );
    await this.step('welcome', 'sendMessage', {
      text: 'Before we start, we need to ensure a few things.', position:
        'bottom'
    },);
    await this.step('welcome', 'sendMessage', {
      text: 'Firstly, we need to see you on the screen.', position:
        'bottom'
    }, );
    await this.step('welcome', 'sendMessage', {
      text: 'Please move around such that you can see your whole body inside the red box.', position:
        'bottom', skipWait: true
    }, );
  ...
  async explainSit2Stand() {
    ...
    await this.step('explain', 'sendMessage', { text : 'Before we start the exercise, please sit down on a chair', position : 'center'})

    await this.step('explain', 'sendMessage', { text: `Grab a chair if you don't have one and please sit down.`, position: 'bottom', skipWait: true })
    ...
    await this.step('explain', 'sendMessage', { text : 'Now lets make this exercise interesting', position : 'center'})
    await this.step('explain', 'sendMessage', { text : 'When you see an ODD number you STAND', position : 'center'})
    await this.step('explain', 'sendMessage', { text : 'Let us try it out...', position : 'center'})
    await this.step('explain', 'sendMessage', { text : 'Let us try it out...', position : 'bottom', skipWait : true})

    await this.step('explain', 'sendPrompt',{ className: 'round', text: '1', position: 'center' } )
    ...
    await this.step('explain', 'sendMessage', { text :'Now when you see an EVEN number you can SIT', position : 'center' })
    await this.step('explain', 'sendMessage', { text : 'Let us give it a try?', position : 'center'})
    await this.step('explain', 'sendMessage', { text : 'Let us give it a try?', position : 'bottom', skipWait : true})
    
    await this.step('explain', 'sendMessage', { text : 'SIT when you see an EVEN number.', position : 'bottom', skipWait : true})
    await this.step('explain','sendPrompt', { className: 'round', text: '12', position: 'center' })
    ...
    await this.step('explain', 'updateAvatar', { name: 'mila', position: 'center' })
    await this.step('explain', 'sendMessage', {text : 'Now we are all set...', position: 'center'})
  ...
  async postPlaySit2Stand() {
    ...
    await this.step('postGame', 'sendMessage', { text: 'Thank you for playing!', position: 'center' })
  ...
  async step(step: ActivityStage, type: 'updateAvatar' | 'sendMessage' | 'sleep' | 'hideAvatar' | 'hideMessage' | 'announcement' | 'sendSpotlight' | 'hideSpotlight' | 'waitForClass' | 'sendPrompt' | 'hidePrompt' | 'startTimer' | 'hideTimer', data?: any,  stepNumber?: number) {
